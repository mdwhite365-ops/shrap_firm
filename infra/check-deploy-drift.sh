...
#
# Usage:
#   ./infra/check-deploy-drift.sh          # report; exit 1 if anything is missing
...
if ! defined=$(docker compose config --services 2>/dev/null | sort); then
  log "ERROR: could not read compose config (wrong directory, or compose not installed)"
  exit 2
fi

if ! running=$(docker compose ps --services --status running 2>/dev/null | sort); then
  log "ERROR: could not list running services"
  exit 2
fi
