...
| 3 | Run over the full corpus; admitted-item lists per bar — **largely DONE 2026-07-31, discovered 2026-09-20**: all three bars ran over the same ~599 items on `qwen3.5:397b` (hard-leg admits A 2/454, B 2/453, C 1/454; every admit one of two USASpending DOE awards). Both `bar_experiment_runs` and `bar_experiment_results` recorded all four invocations the whole time; the results were simply never written up. See `docs/research/archetype-bar-results.md`. **Now complete (#264):** the `kimi-k3` replay of the same 599 items scored 3 hard admits against `qwen3.5:397b`'s 2 — indistinguishable at ±1.5 SE, so the model is not the constraint either. Both of the experiment's variables have been tested. What the data does show is a **source** effect the spec did not anticipate: `sec-edgar` and `arxiv` are 95% of the items and admitted **zero** under both models, while every admit came from the three sources that are 1.7% of the corpus. Superseded note — **partial 2026-09-20**: 200-item stratified pilot run on `kimi-k3`, results in `docs/research/archetype-bar-results.md`. Full run blocked on quota (63,693 completions ≈ 10.6 weekly allowances, measured) and on whether the hypothesis still warrants it — DQ-006's named exemplar flipped on a *model* change, not a bar change. |
| 4 | Mike's ruling; `world-changer-archetypes.md` updated first, then the mirror and the prompt — **evidence assembled 2026-09-22 in `docs/research/archetype-bar-ruling.md`**: on 425 EDGAR filings, B admits 14, A admits 6, C admits 3, and the disagreements are perfectly nested (B-only 8 / A-only 0, McNemar p = 0.008). Two stated limits: a 14-admit base, and EDGAR only. |
