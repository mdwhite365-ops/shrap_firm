...
**Role.** Classifies market regimes, generates and validates trading strategies, and maintains the Bayesian edge estimates that keep the Trading Floor calibrated. The Regime Classifier is the source of truth for current market state — its `regime.updated` events drive strategy activation on the Trading Floor and ground hypothesis generation within Research. The Regime Researcher produces historical-analog context on a slower cadence for deeper regime understanding. The kill rate on generated hypotheses is expected to exceed 90% — that is a feature, not a failure.

**Key agents.** Regime Classifier (produces statistical-state regime classification — VIX level, trend, breadth, dispersion, term structure — on a regular cadence; publishes `regime.updated`), Regime Researcher (produces historical-analog regime classification on a weekly or macro-inflection cadence; writes regime profiles to `docs/regimes/`; publishes `regime.analog.updated`), Hypothesis Generator (proposes strategies given regime + universe context), Strategy Evaluator (runs walk-forward backtests on VectorBT PRO, applies overfitting controls), Bayesian Updater (maintains posterior edge estimates from live results), Strategy Librarian (maintains the strategy registry in PostgreSQL, tracks lifecycle state).

**Primary interfaces.** Reads: universe profiles from `docs/universe/`, strategy registry from PostgreSQL, macro and market data from NautilusTrader and external sources. Writes: regime classification records, backtest results, and strategy records to PostgreSQL; regime profiles to `docs/regimes/`. Publishes: `regime.updated`, `regime.analog.updated`, `strategy.promoted`, `strategy.retired`, `strategy.hypothesis.generated`. Consumes: `intelligence.signal` (for hypothesis grounding).

**LLM tier.** Regime Classifier: no LLM — statistical computation. Regime Researcher: cloud (Claude Sonnet 4.6) — historical analog synthesis requires judgment. Hypothesis Generator: cloud (Claude Sonnet 4.6) during sprint. Strategy Evaluator: no LLM — deterministic VectorBT PRO execution. Bayesian Updater: no LLM — statistical computation. Strategy Librarian: local (Qwen 9B) sufficient for registry maintenance tasks.

**Sprint scope.** Regime Classifier operational by end of month 2 — prerequisite for strategy activation on the Trading Floor. Hypothesis Generator and Strategy Evaluator operational by end of month 2. Regime Researcher and Bayesian Updater by end of month 3. Full promotion pipeline by end of month 3.
